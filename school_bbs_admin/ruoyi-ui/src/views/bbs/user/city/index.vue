...
<script>
import { listCity, getCity, delCity, addCity, updateCity } from "@/api/bbs/user/city";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除城市字典表编号为"' + ids + '"的数据项？').then(function() {
        return delCity(ids);
      }).then(() => {
        this.getList();
